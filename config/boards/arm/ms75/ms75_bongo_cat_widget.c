...
#include "ms75_bongo_cat_widget.h"
...
void state_widget_wpm(struct ms75_bongo_cat_widget *widget, int wpm) {
    LOG_DBG("anim state %d", current_anim_state);
    if (wpm < CONFIG_MS75_BONGO_CAT_IDLE_LIMIT) {
    ...
    } else if (wpm < CONFIG_MS75_BONGO_CAT_SLOW_LIMIT) {
...
int wpm_status_listener(const zmk_event_t *eh) {
    struct ms75_bongo_cat_widget *widget;
...
ZMK_LISTENER(ms75_bongo_cat_widget, wpm_status_listener)
ZMK_SUBSCRIPTION(ms75_bongo_cat_widget, zmk_wpm_state_changed);
